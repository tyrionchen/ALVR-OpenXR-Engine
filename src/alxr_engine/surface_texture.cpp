	...
	nativeWindow( NULL)
{
	...
	// 创建Surface对象的局部引用
	obj = jni->NewObject( surfaceClass, surface_constructor, javaObject );
	javaObjectSurface = jni->NewGlobalRef( obj );
	// 有全局引用了，删除局部引用
	jni->DeleteLocalRef( obj );

	Log::Write(Log::Level::Info, Fmt("cyyyyy SurfaceTexture got method updateTexImageMethodId:%p, getTimestampMethodId:%p, setDefaultBufferSizeMethodId:%p javaObjectSurface:%p",
		updateTexImageMethodId, getTimestampMethodId, setDefaultBufferSizeMethodId, javaObjectSurface));

	nativeWindow = ANativeWindow_fromSurface(jni, GetJavaObjectSurface());

	Log::Write(Log::Level::Info, Fmt("cyyyyy SurfaceTexture ANativeWindow_fromSurface nativeWindow:%p", nativeWindow));
}
...
ANativeWindow* SurfaceTexture::GetANativeWindow() {
	return nativeWindow;
}
